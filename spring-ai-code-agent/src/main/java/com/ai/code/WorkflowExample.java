...
public class WorkflowExample {
    ...
    public static void main(String[] args) {
        ...
        if (chatModel == null) {
            ...
            System.err.println("请设置 ANTHROPIC_AUTH_TOKEN 环境变量");
            return;
        }
    ...
    public void example9_multiAgentResearchWorkflow() throws Exception {
        ...
        record SummaryRequest(String content) {}

        // 创建工具（示例）
        ToolCallback searchTool = FunctionToolCallback
                .builder("search", (SearchRequest request, ToolContext context) -> RESEARCH_RESULT)
                .description("搜索工具，用于收集研究信息")
                .inputType(SearchRequest.class)
                .build();

        ToolCallback analysisTool = FunctionToolCallback
                .builder("analysis", (AnalysisRequest request, ToolContext context) -> "分析结果")
                .description("分析工具，用于深入分析数据")
                .inputType(AnalysisRequest.class)
                .build();

        ToolCallback summaryTool = FunctionToolCallback
                .builder("summary", (SummaryRequest request, ToolContext context) -> "总结结果")
                .description("总结工具，用于提炼结论")
                .inputType(SummaryRequest.class)
                .build();
...
//				.systemPrompt("你负责分析输入内容的性质，并选择最合适的专家处理。")
        ...
        class PostprocessorNode implements NodeAction {
            ...
            public Map<String, Object> apply(OverAllState state) throws Exception {
                ...
                if (technicalOutput.isPresent()) {
                    String content = extractContent(technicalOutput.get());
                    if (content != null && !content.isEmpty()) {
                        mergedResult.append("[技术文档] ").append(content).append("\n");
                    }
                }
                ...
                if (marketingOutput.isPresent()) {
                    String content = extractContent(marketingOutput.get());
                    if (content != null && !content.isEmpty()) {
                        mergedResult.append("[营销文案] ").append(content).append("\n");
                    }
                }
                ...
                if (customerOutput.isPresent()) {
                    String content = extractContent(customerOutput.get());
                    if (content != null && !content.isEmpty()) {
                        mergedResult.append("[客户服务] ").append(content).append("\n");
                    }
                }
            ...
            // 辅助方法：从不同类型的输出中提取文本内容
            private String extractContent(Object output) {
                if (output == null) {
                    return null;
                }
                if (output instanceof GraphResponse) {
                    // GraphResponse 格式：需要从 resultValue 中提取
                    try {
                        GraphResponse response = (GraphResponse) output;
                        Optional<Object> resultValue = response.resultValue();
                        if (resultValue.isPresent() && resultValue.get() instanceof Map) {
                            Map<String, Object> resultMap = (Map<String, Object>) resultValue.get();
                            // 尝试获取包含 _output 或 output 的键
                        ...
                        System.err.println("提取GraphResponse内容失败: " + e.getMessage());
                    }
                } else if (output instanceof AssistantMessage) {
                    // AssistantMessage 格式：直接从 text 属性提取
                    try {
                        AssistantMessage message = (AssistantMessage) output;
                        return message.getText();
                    } catch (Exception e) {
                        System.err.println("提取AssistantMessage内容失败: " + e.getMessage());
                    }
                } else if (output instanceof Map) {
                    // Map 格式：直接返回整个 Map 的文本表示
        ...
        NodeOutput result1 = compiledGraph.stream(
                Map.of("input", "Spring AI Alibaba框架和spring ai框架的区别，用150字以内介绍？")
        ).doOnNext(output -> {
            if (output instanceof StreamingOutput<?> streamingOutput) {
                if (streamingOutput.message() != null) {
                    // 根据agent属性分类输出
