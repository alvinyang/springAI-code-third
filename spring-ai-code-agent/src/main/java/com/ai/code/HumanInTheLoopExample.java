...
public class HumanInTheLoopExample {
    ...
    public void example1_basicConfiguration() {
        ...
        record ReadDataRequest(String table) {}

        // 创建工具回调（示例）
        ToolCallback writeFileTool = FunctionToolCallback.builder("write_file", 
                (WriteFileRequest request, ToolContext context) -> "文件已写入")
                .description("写入文件")
                .inputType(WriteFileRequest.class)
                .build();

        ToolCallback executeSqlTool = FunctionToolCallback.builder("execute_sql", 
                (ExecuteSqlRequest request, ToolContext context) -> "SQL已执行")
                .description("执行SQL语句")
                .inputType(ExecuteSqlRequest.class)
                .build();

        ToolCallback readDataTool = FunctionToolCallback.builder("read_data", 
                (ReadDataRequest request, ToolContext context) -> "数据已读取")
                .description("读取数据")
                .inputType(ReadDataRequest.class)
                .build();
    ...
    public void example2_approveDecision() throws Exception {
        ...
        // 定义工具输入类型
        record PoemRequest(String topic) {}

        ToolCallback poetTool = FunctionToolCallback.builder("poem", 
                (PoemRequest request, ToolContext context) -> "春江潮水连海平，海上明月共潮生...")
                .description("写诗工具")
                .inputType(PoemRequest.class)
                .build();
    ...
    public void example3_editDecision() throws Exception {
        ...
        ToolCallback executeSqlTool = FunctionToolCallback.builder("execute_sql", 
                (ExecuteSqlRequest request, ToolContext context) -> "SQL执行结果")
                .description("执行SQL语句")
                .inputType(ExecuteSqlRequest.class)
                .build();
    ...
    public void example4_rejectDecision() throws Exception {
        ...
        // 定义工具输入类型
        record DeleteDataRequest(String target) {}

        ToolCallback deleteTool = FunctionToolCallback.builder("delete_data", 
                (DeleteDataRequest request, ToolContext context) -> "数据已删除")
                .description("删除数据")
    ...
    public void example5_multipleTools() throws Exception {
        ...
        if (result.isPresent() && result.get() instanceof InterruptionMetadata) {
            ...
            System.out.println(outputOptional.get().state().value("result_msg").get());
    ...
    public void example6_workflowWithHumanInTheLoop() throws Exception {
        ...
        ToolCallback searchTool = FunctionToolCallback
                .builder("search", (SearchRequest request, ToolContext context) -> 
                        "搜索结果：AI Agent是能够感知环境、自主决策并采取行动的智能系统。")
        ...
        ReactAgent qaAgent = ReactAgent.builder()
                .name("qa_agent")
                .model(chatModel)
                .instruction("你是一个问答专家，负责回答用户的问题。请使用search工具。\n用户问题：{cleaned_input}")
                .outputKey("qa_result")
                .saver(saver)
